...
urlpatterns = [
    ...
    re_path('^api/case/(?P<pk>[0-9]+)/transfers/$', caseviews.CaseTransferAPIView.as_view({'get': 'retrieve'}), name='casetransferapi-detail'),
    ...
    re_path('^api/case/(?P<caseid>\d+)/assign/$', caseviews.assign_case, name='assign_case'),
    re_path('^api/case/(?P<caseid>\d+)/unassign/$', caseviews.unassign_case, name='unassign_case'),
