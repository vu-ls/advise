...
urlpatterns = [
    ...
    re_path('^case/post/diff/(?P<revision_id>[0-9]+)/$', caseviews.PostDiffView.as_view(), name='diff'),
    re_path('^api/case/(?P<caseid>\d+)/threads/archived/$', caseviews.ArchivedThreadView.as_view({'get': 'list'}), name='archived'),
    re_path('^api/case/(?P<pk>\d+)/posts/$', post_list, name='postlistapi'),
    re_path('^api/case/post/(?P<pk>[0-9]+)/$', post_detail, name='postapi'),
