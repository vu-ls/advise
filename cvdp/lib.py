...
def add_new_case_participant(thread, name, user, role):
    ...
    if ((created or role_change) and role == "owner"):
        email_context = {'url': thread.case.get_absolute_url(), 'case': thread.case.caseid, 'template': "case_assignment", 'assignee': user.screen_name}
...
def notify_case_participant(participant, subject, content, user):
    ...
    participant.save()
    email_context={'url': participant.case.get_absolute_url(), 'case': participant.case.caseid}
