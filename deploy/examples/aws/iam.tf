...
resource "aws_iam_role_policy" "ecs_task_app_attachments_bucket_access_policy" {
  name   = "${local.name_prefix}-ecs-task-app-attachments-bucket-access-policy-${local.unique_id}"
  policy = data.aws_iam_policy_document.app_attachments_bucket_access_role.json
  role   = module.app_service.ecs_task_role_id
}

resource "aws_iam_role_policy" "ecs_task_app_media_bucket_access_policy" {
  name   = "${local.name_prefix}-ecs-task-app-media-bucket-access-policy-${local.unique_id}"
  policy = data.aws_iam_policy_document.app_media_bucket_access_role.json
  role   = module.app_service.ecs_task_role_id
}
...
data "aws_iam_policy_document" "app_attachments_bucket_access_role" {
  statement {
    effect = "Allow"
    actions = [
      "s3:PutObject",
    ...
    resources = [
      "${module.app_attachments_bucket.arn}",
      "${module.app_attachments_bucket.arn}/*"
    ]
  }
}

data "aws_iam_policy_document" "app_media_bucket_access_role" {
  statement {
    effect = "Allow"
    actions = [
      "s3:PutObject",
