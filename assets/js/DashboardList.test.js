...
describe("Dashboard Component", () => {
  ...
  it("should show loading dots while loading", async () => {
    ...
    await waitFor(() => {
      expect(container.getElementsByClassName("lds-spinner").length).toBe(2);
    });
  });
  ...
  it("should render cases when api responds", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/added Affected status/)).toBeInTheDocument();
    });
  });
  ...
  it("should filter when input is changed - no results", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/You have no active cases/)).toBeInTheDocument();
    });
  });
  ...
  it("should go to case view when clicked", async () => {
    const history = createMemoryHistory({initialEntries:["/advise/dashboard"]});
    history.push = jest.fn();
    
    render(
      <MemoryRouter initialEntries={["/advise/dashboard", "/advise/cases/:id"]} initialIndex={0}>     
        <DashboardApp />
      </MemoryRouter>
    );
